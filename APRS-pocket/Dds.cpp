/*
  ...
  VREF_DAC0REFSEL_0V55_gc  0.55 V
  VREF_DAC0REFSEL_1V1_gc  1.1 V
  VREF_DAC0REFSEL_2V5_gc  2.5 V
  VREF_DAC0REFSEL_4V34_gc 4.34 V
  VREF_DAC0REFSEL_VDD_gc  VDD (tension d'alim)
...
Dds::Dds(float _splFreq, int _syncLed)
{
  syncLed = _syncLed;
  splFreq = _splFreq;
  accumulateur = 0;
...
void Dds::begin() {
  ...
  //TCA0.SINGLE.PER = 1515;//13200Hz
  TCA0.SINGLE.PER = 372;//26400.0
...
*/
ISR(TCA0_OVF_vect) {
  if (Dds::anchor->ddsEn) {   //irq dds ?
    Dds::anchor->interruption();
  }
  digitalWrite(Dds::anchor->syncLed, digitalRead(Dds::anchor->syncLed) ^ 1);
  Dds::anchor->cptMs++;
  if (Dds::anchor->cptMs == 26) {
    Dds::anchor->countMs++;
    Dds::anchor->cptMs = 0;
  }
